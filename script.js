...
const renderKeyboard = () => {
  ...
  for (let i = 0; i < keyData.length; i++) {
    ...
    for (let j = 0; j < keyData[i].length; j++) {
      ...
      } else if (state.shift == true) {
        if (language == 'eng') {
          key.innerText = keyData[i][j].eng.capsShift;
        } else if (language == 'ru') {
          key.innerText = keyData[i][j].ru.capsShift;
        }
      } else if (language == 'eng') {
        key.innerText = keyData[i][j].eng.caps;
      } else if (language == 'ru') {
...
const shiftActive = () => {
  document.addEventListener('keydown', (e) => {
    ...
    if (e.code == 'ShiftLeft' || e.code == 'ShiftRight') {
      ...
      if (state.caps == false) {
        ...
        renderKeyboard();
        const shift = document.querySelector(`.${e.code}`);
        shift.classList.add('key_active');
      } else {
  ...
  document.addEventListener('keyup', (e) => {
    if (e.code == 'ShiftLeft' || e.code == 'ShiftRight') {
      state.shift == true ? state.shift = false : null;
      if (state.caps == false) {
        state.case = 'lowerCase';
        renderKeyboard();
      } else {
